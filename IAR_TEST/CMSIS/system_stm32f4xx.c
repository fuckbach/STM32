...
void SystemInit(void)
{
  ...
  RCC->PLLCFGR = RCC_PLLCFGR_PLLSRC_HSE;//PLL source - HSE M=4, N = 168, P = 2, Q = 7;for sysclk = 168MHz
  RCC->PLLCFGR |= RCC_PLLCFGR_PLLM_2 ;
  RCC->PLLCFGR |=  RCC_PLLCFGR_PLLN_3 | RCC_PLLCFGR_PLLN_5 | RCC_PLLCFGR_PLLN_7;
  RCC->PLLCFGR &= ~RCC_PLLCFGR_PLLP;
  RCC->CR |= RCC_CR_PLLON;//PLL ON
  while((RCC->CR & RCC_CR_PLLRDY)==0);//ready start PLL
  RCC->CFGR|= RCC_CFGR_SW_PLL;// choose PLL for clk
